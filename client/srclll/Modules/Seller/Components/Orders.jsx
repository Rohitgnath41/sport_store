            ...
            {bookings.filter(booking => userProducts.includes(booking.product_id)).map((booking, index) => (
              <TableRow key={booking.id}>
                ...
                {booking.status == "Ordered" ? <TableCell>
                  <Button variant="contained" color="secondary" style={{backgroundColor:"green",marginRight:"5px"}} onClick={()=>handleAccept(booking.id)}>Accept</Button>
                  <Button variant="contained" color="secondary" onClick={()=>handleReject(booking.id)}>Reject</Button>
